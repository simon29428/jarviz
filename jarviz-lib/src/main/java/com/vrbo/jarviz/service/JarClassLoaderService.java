...
public class JarClassLoaderService implements ClassLoaderService {
     ...
     * qualified class names in the format of "com.foo.bar.MyClass".
     *
     * @param artifact        The artifact to be scanned.
     * @param classNameFilter A filter to conditionally select classes in the top
     *                        level package. Class name will be in the format
     *                        "com.foo.bar.MyClass".
     * @return A list of fully qualified class names for all the classes loaded
     *         (matching the above criteria).
     */
    @Override
    public List<ShadowClass> getAllClasses(@Nonnull Artifact artifact, @Nonnull final Predicate<String> classNameFilter) {
        ...
        final String jarFilePath = jarFile.toString();
        final ClassPath classPath;
        try {
            final ClassLoader classLoader = creatClassLoaderForJar(jarFile);
        ...
        final String prefix = fileNameToJarPrefix(jarFile);
        final Predicate<ClassPath.ClassInfo> predicateClassesOnlyFromJar = classInfo -> classInfo.url().toString().startsWith(prefix);
        // select classes only from the Jar
        final List<ClassPath.ClassInfo> classInfoList = classPath.getAllClasses().stream().filter(predicateClassesOnlyFromJar)
     ...
     * parent. Then this class loader will load all the classes from the given Jar
     * file. The bootstrap classloader is mainly responsible for loading JDK
     * internal classes, typically rt.jar and other core libraries located in
     * $JAVA_HOME/jre/lib directory. We are assuming here that the loaded classes
     * from the Jar have not been already loaded in the bootstrap classloader (to
     * prevent collisions). For each Jar, a new classloader is created before the
     * analysis and that class loader is destroyed (unloaded) along with the loaded
     * classes from the Jar after the analysis.
     *
     ...
     */
    private static ClassLoader creatClassLoaderForJar(@Nonnull final Path jarFileName) throws MalformedURLException {
        final URL jarURL = jarFileName.toUri().toURL();
     ...
     */
    static String fileNameToJarPrefix(@Nonnull final Path jarFileFullPath) {
        try {
            return "jar:" + jarFileFullPath.toUri().toURL() + "!";
        } catch (MalformedURLException e) {
        }
        return "jar:" + jarFileFullPath.toUri() + "!";
